...
(function ($) {
    ...
    function SlickGrid(container, data, columns, options) {
        ...
        function init() {
            ...
            $headerL = $("<div class='slick-header-columns slick-header-columns-left' style='width:500000px; left:-1000px' />").appendTo($headerScrollerL);
            $headerR = $("<div class='slick-header-columns slick-header-columns-right' style='width:500000px; left:-1000px' />").appendTo($headerScrollerR);
        ...
        function setOverflow() {
            $viewportTopL.css({
                'overflow-x': ( options.frozenColumn > -1 ) ? ( hasFrozenRows ) ? 'hidden' : 'hidden' : ( hasFrozenRows ) ? 'hidden' : 'hidden',
                'overflow-y': ( options.frozenColumn > -1 ) ? ( hasFrozenRows ) ? 'hidden' : 'hidden' : ( hasFrozenRows ) ? 'hidden' : 'hidden'
            });
        ...
        function getColumnCssRules(idx) {
            if (!stylesheet) {
				if(typeof $style[0].sheet != 'undefined') {
        ...
        function resizeCanvas() {
            ...
            viewportTopH = paneTopH - topPanelH - headerRowH - $headers.outerHeight();
        ...
        function _handleScroll(isMouseWheel) {
            ...
            if (hScrollDist) {
				...
				//Fix chrome bug that causes infinite scroll by 1px at certain zoom levels -SGE 9/29/2014
				if($viewportScrollContainerX[0].scrollLeft !== scrollLeft)
					$viewportScrollContainerX[0].scrollLeft = scrollLeft;
